...
import chapter07.backtrack.UVA1354;
...
public class EightNum {
    ...
    static int hashSize = 1000003;
    static int[] head = new int[hashSize];
    static int[] next = new int[maxState];

    static void init_lookup_table() {
        head = new int[hashSize];
    }

    static int hash(State state) {
        int v=0;
        for (int i = 0; i < 9; i++) {
            v = v * 10 + state.graph[i];
        }
        return v % hashSize;
    }
