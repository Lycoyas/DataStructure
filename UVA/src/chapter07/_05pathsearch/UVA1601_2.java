    ...
    static int bfs() {
        ...
        while (!q1.isEmpty()||!q2.isEmpty()) {
            if (!q1.isEmpty()) {
                ...
                if(dist2[a][b][c]!=-1) return dist1[a][b][c]+dist2[a][b][c];
            ...
            if (!q2.isEmpty()) {
                int u = q2.removeFirst();
                int a = (u >> 16) & 0xff,b = (u >> 8) & 0xff,c = u & 0xff;
                if(dist1[a][b][c]!=-1) return dist1[a][b][c]+dist2[a][b][c];
                for (int i = 0; i < deg[a]; i++) {
                    int a2 = G[a][i];
                    for (int j = 0; j < deg[b]; j++) {
                        int b2 = G[b][j];
                        ...
                        for(int k = 0; k < deg[c]; k++) {
                            int c2 = G[c][k];
                            if(conflict(a, c, a2, c2) || conflict(b, c, b2, c2)) continue;
                            if(dist2[a2][b2][c2] == -1) {    //等于-1说明没有访问过该状态，就要压入队列
                                dist2[a2][b2][c2] = dist2[a][b][c] + 1;
                                q2.addLast(mm(a2, b2, c2));
                            }
                        }
